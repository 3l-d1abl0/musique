			...
			<div id="main">
				<div id="display" class="fl">
						...
						$category=$_GET['category'];
							...
							$query="select * from flute where category='$category' and status=2";
							$result=mysqli_query($con,$query);
							while($row=mysqli_fetch_array($result))
							{
								$name=strtok($row[0]," ");
								echo '<tr><td><br><a href="booking.php?name='.$row[0].'&id='.$row[1].'&tn=flute&sub1=submit"><img src="instrument/flute/'.$category.'/'.$name.'.jpg" width="300" height="180"></a></td><td>'.$row[0].'<br>Price: Rs '.$row[3].'</td></tr>';
							}
							echo '</table></div><hr>';		
						?>
				</div>
				...
				<div id="side2" class="fl">
				      <div id="joinus"><a href="register.php"><center><br>Join Us</center></a></div>
						 ...
						 <br><br><center><font size="4" color="#f5f0ec">Login</font></center>
								<form name="form1" action="flutesection.php" method="post">
		...
		else
		{
			...
			if($result)
			{
				...
				if($row[0]==$pass)
				{
					$_SESSION['name']='user';
					$_SESSION['id']=$email;
					header('Location: flutesection1.php');	
				}	
				else
				header('Location:flutesection.php');
			}
			else
			header('Location:flutesection.php');
		}
